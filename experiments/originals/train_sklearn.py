...
from utils import load_cfg, makedirs
...
def build_xy(cfg):
    ...
    chs = cfg['muse_channels'] if cfg['device']['type']=='muse_ble' else cfg['openbci_channels']
    ...
    for arr, m in zip(Xs, meta):
        ...
        X_list.append(X_feat); y_list.append(np.repeat(lab, len(X_feat)))
    X = pd.concat(X_list, ignore_index=True)
    y = np.concatenate(y_list)
    return X, y

def train_eval_save():
    cfg = load_cfg(); makedirs(cfg['paths']['artifacts_dir'])
